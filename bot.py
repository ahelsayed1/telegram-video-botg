...
async def handle_broadcast_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if update.message and update.message.reply_to_message:
            ...
            if replied_text and "إذاعة من الإدارة:" in replied_text:
                user_id = update.effective_user.id
                ...
                if user:
                    db.log_activity(
                        ...
                        details=f"reply: {update.message.text[:50] if update.message.text else 'بدون نص'}"
                    )
...
🆔 المعرف: {user_id}
💬 الرد: {update.message.text[:100] if update.message.text else 'رسالة ميديا'}
"""
        ...
        logger.error(f"خطأ في handle_broadcast_reply: {e}")
...
def main():
    ...
    # فحص حالة قاعدة البيانات قبل التشغيل
    try:
        db_status = db.get_database_info()
